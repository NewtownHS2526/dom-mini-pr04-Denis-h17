...
function movePenguin(penguinElement, columnPosition) {
  if (columnPosition <= 6) {
    penguinElement.style.gridColumn = columnPosition;
  }
}
...
function checkWinner(penguinName) {
  if (penguinName === "Tux" && tuxPosition >= 6) {
    winnerDisplay.textContent = "Tux got the fish! 🐟";
    disableAllButtons();
  } else if (penguinName === "Daisy" && daisyPosition >= 6) {
    winnerDisplay.textContent = "Daisy got the fish! 🐟";
    disableAllButtons();
  } else if (penguinName === "Rocky" && rockyPosition >= 6) {
    winnerDisplay.textContent = "Rocky got the fish! 🐟";
    disableAllButtons();
  }
}
